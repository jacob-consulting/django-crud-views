...
- **Shell hazard — cwd does not stay anchored to the worktree between commands.** A subagent executing this plan does not inherit worktree-pinning the way the controlling session does: a `cd <worktree>` in one Bash call does not carry over to the next call, and the default reset target is the root checkout `/home/alex/projects/alex/django-crud-views`, not this worktree. (Task 1's implementer used relative commands like `cd tests && pytest ...` and its final `git commit` landed on `main` in the root checkout instead of this branch — caught and repaired afterward by cherry-picking the commit onto this branch and reverting the stray commit from root `main`.) Every command in every task — not just the first one in a step — must begin with the absolute path: `cd /home/alex/projects/alex/django-crud-views/.claude/worktrees/feat+integrate-conditional-field-groups && unset VIRTUAL_ENV && ...`.
