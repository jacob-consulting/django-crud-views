...
**Why introspection, not a registry:** every legitimate config attribute is already declared on a package class — most with a default (e.g. `cv_message_template: str | None = None` and `cv_message_template_code: str | None = None` at `base.py:82-83`), a few annotation-only with no default (e.g. `cv_formsets: FormSets` on `FormSetMixin`). The known-set therefore derives itself from the class hierarchy — scanning both `vars()` (attributes with defaults) and `__annotations__` (annotation-only declarations) — and stays correct as attributes are added or renamed, with no separate list to maintain.

### Known-set / suspect / unknown computation

- **Known set (package-wide vocabulary)** — `cv_*` names declared by *any* class whose `__module__` starts with `"crud_views"`, collected from both `vars()` (attributes with defaults) and `__annotations__` (annotation-only declarations like `cv_formsets: FormSets`). The vocabulary is gathered from the union of the context view's own MRO **and every registered view's MRO** (via the ViewSet registry). This makes the check pure *typo detection* — "is this a real `crud_views` attribute name anywhere?" — so a real attribute set on a view that does not inherit its declaring mixin (e.g. `cv_formsets`, which the package's own `check_conditional` reads generically via `getattr` on any view) is recognized, not flagged. The `"crud_views"` prefix covers core plus `crud_views_workflow`, `crud_views_polymorphic`, `crud_views_guardian`, and any `crud_views_widget_*` community extension. The registry portion is cached (keyed on registry size) so per-view checks stay linear. Whether an attribute is *applicable to this view type* is a separate concern handled by the existing typed checks (E204, etc.); W280 only answers whether the name exists in the package vocabulary.
- **Suspects** — `cv_*` **data attributes** in the `__dict__` of every *non-package* class in the context view's MRO (user code — **all** such classes, so typos on a shared user mixin are caught too, not only on the leaf view). Data attributes are values that are not callables or descriptors (`property`, `classmethod`, `staticmethod`, etc.). `cv_message = "x"` is a suspect; `def cv_helper(self): ...` is skipped.
- **Allowlist** — the union of `cv_check_ignore_attributes` across the MRO (see escape hatch).
- **Unknown** — `suspects − known − allowlist`. One warning per remaining name. The suggestion pool is the *data-attribute* subset of the known set, so a near-match never points at a method.
